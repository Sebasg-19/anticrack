...
countries=(cn ru ir kp by sy ve cu pk kh mm ni er ml cf bf ne gl al be bg ca hr cz dk ee fi fr de gr hu is it lv lt lu me nl mk no pl pt ro sk si es se tr gb us in br za ua nsa fbi gchq mi6)
...
function enable_country() {
  ...
  if [ "$c" == "nsa" ]; then
    ...
    ips=("${mi6_ips[@]}")
    # Block domains
    for d in sis.gov.uk www.sis.gov.uk mi6.gov.uk www.mi6.gov.uk; do
      if ! grep -q "^0.0.0.0 $d $BLOCK_MARKER" "$HOSTS_FILE"; then
        echo "0.0.0.0 $d $BLOCK_MARKER" >> "$HOSTS_FILE"
      fi
    done
